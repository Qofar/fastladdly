...
var SelectorWithCart = Class.create().extend({
	...
	_updateCart : function(element){
		...
		var chk = hasClass(element.parentNode.parentNode, "selected");
		if(!chk){
...
Manage.Item = {
	...
	do_move : function(){
		...
		if(to === "Create New Folder") {
			var to = prompt("Folder Name","");
			if(!to) return;
		}
	...
	toggle_notify: function(){
		...
		api.post({
			...
			message("Changed the notification settings")
		});
		update("manage_item")
	},
	...
	unsubscribe: function(){
		...
		foreach(ids,function(sid,n){
			...
			api.delete({},function(){
				l--;
				message("Removing feeds: "+l+" items to go");
				if(l == 0){
...
updater("move_to", function(){
	...
	this.options[0] = new Option("Create New Folder","Create New Folder");
	this.options[1] = new Option("Uncategolized","");
	var op = this.options;
	folder.names.map(function(v,i){
		op[i+2] = new Option(v,v);
	})
});
...
updater("manage_folder", function(){
	...
	$("manage_folder").innerHTML = [
		'<li class="button ' + (MF.folder_id == 0 ? 'selected' : '')  + '" _onclick="MF.select(0)">Uncategolized</li>',
...
updater("update_folder",function(){
	...
	ajaxize("delete_form",{
		before: function(param){
			var c = confirm("Are you sure to remove "+folder.id2name[MF.folder_id]+"?  (Items inside won't be removed)");
			return c ? true : false;
		},
		after: function(res,req){
			var fn = folder.id2name[MF.folder_id];
			message(fn + " deleted");
			MF.folder_id = null;
			folder = null;
...
Manage.Folder = {
	...
	rename_folder : function(callback){
		...
		var api = new API(feedly.BASE+"/categories/" + encodeURIComponent(folder_id));
		api.post({label : new_name},function(){
	...
	delete_folder : function(callback){
		var folder_id = MF.folder_id;
		if(!folder_id) return false;
		var c = confirm("Are you sure to remove "+folder.id2name[folder_id]+"?  (Items inside won't be removed)");
		if(!c) return false;
