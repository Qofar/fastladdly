// API
var ApiKey = feedly.access_token;
...
function show_error(){
	...
	// reload ApiKey
    var apikey = LDR_getApiKey();
	if (API.StickyQuery["ApiKey"] != apikey) {
    	API.StickyQuery["ApiKey"] = apikey;
	}
	update("error_window")
}
...
function show_all_mouseover(){
	...
	State.help_message = [
		...
		(Config.show_all ? 'disabled':'enabled')
	].join("");
	update("help_window");
}
...
LDR.trigger = new Class.Trigger(
	...
	// sub contents
	'AFTER_INIT_CONFIG','AFTER_INIT_GUIDE','AFTER_INIT_MANAGE','AFTER_INIT_RANKING',
	// EVENT
	'WINDOW_RESIZE',
...
function setup_hook(){
	...
	register_hook('AFTER_INIT_RANKING', guide_fix);
	...
	register_hook('AFTER_CONFIGLOAD', function(){
		...
		State.reloadTimer = setInterval(function(){
			...
			if((new Date - State.LastUserAction) > freq){
				Control.reload_subs();
...
updater("total_unread_count", function(){
	...
	this.innerHTML = "[[feed_count]] feeds&nbsp;&nbsp;|&nbsp;&nbsp;[[count]] items".fill(param);
	if(!State.guest_mode){
		document.title = "Fastladder ([[count]])".fill(param);
	}
});
...
Config.loadform = function(form){
	var json = Object.toJSON(Config);
	Form.fill(form, json);
};
...
function setup_event(){
	...
	ClickEvent.add('[_onmouseup]', function(){
		var rel = this.getAttribute("_onmouseup");
		eval(rel);
	});
	ClickEvent.add('[_onclick]', function(){
		var rel = this.getAttribute("_onclick");
		eval(rel);
	});
	ClickEvent.add('[_onmousedown]', function(){
		var rel = this.getAttribute("_onmousedown");
		eval(rel);
	});
...
function print_navi(){
	...
	if(prev_id){
		buf.push([
			'<div class="button prev" _onclick="Control.read_prev_subs()">',
			'<span class="navi_text">≪ [[title]]</span></div>'
		].join("").fill( subs_item(prev_id) ))
	}
	if(next_id){
		buf.push([
			'<div class="next button" _onclick="Control.read_next_subs()">',
			'<span class="navi_text">[[title]] ≫</span></div>'
...
function unsubscribe(sid,callback){
	...
	var api = new API(feedly.BASE+"/subscriptions/" + encodeURIComponent(sid));
	callback = callback || Function.empty;
	var info = subs_item(sid);
	confirm(
		info ? "Are you sure to remove [[title]] from your subscription?".fill(info) : "Are you sure to unsubscribe this feed?"
...
function touch_all(id){
	...
	if(info.unread_count == 0){
		...
		info.unread_count = 0;
		var api = new API(feedly.BASE+"/markers");
		api.post({
			action: "markAsRead",
			type: "feeds",
			lastReadEntryId: info.tochid,
			feedIds: [
				info.subscribe_id
			]
		}, function(){
...
Pin.extend({
	...
	add: function(url,title,info,id){
		...
		queue.push(function(){
			if(can_popup){
				(max_pin).times(function(){
					var p = self.shift();
					p && new API(feedly.BASE+"/tags/" + encodeURIComponent("user/"+feedly.id+"/tag/global.saved") + "/" + encodeURIComponent(p.id)).delete();
				})
				self.update_view();
			} else {
				message('cannot_popup')
...
Pinsaver.extend({
	add: function(url,title,info,id){
		if(!Config.use_pinsaver) return;
		var api = new API(feedly.BASE+"/tags/" + encodeURIComponent("user/"+feedly.id+"/tag/global.saved"));
		api.put({
			entryId: id
		})
	},
...
function get_active_item(detail){
	...
	var divs = $("right_body").getElementsByClassName("item_title");
	// for Opera9 beta
...
var Control = {
	...
	pin_hover: function(e){
		...
		var write_menu = function(){
			...
			menu.add([
				...
				'List vie (',pin.pins.length,' items)</span>'
			].join(""));
			foreach(pin.pins,function(v,i){
				...
				var item = tmpl({
					...
					icon  : v.icon || 'http://image.reader.livedoor.comimg/icon/default.gif'
				});
				menu.add(item);
			});
			menu.add([
	...
	feed_page: function(num){
		...
		var api_url = feedly.BASE+"/streams/contents?streamId=" + encodeURIComponent(sid) + "&count="+c+"&unreadOnly=false&ranked=newest";
		var api = new API(api_url);
		api.onload = function(json){
			var json = feedly.feed2fastladder(sid,json)
			print_feed(json);
			// リクエストよりも件数が少ない場合
	...
	next_item_offset: function(){
		...
		// var divs = $("right_body").getElementsByTagName("h2");
		var divs = $("right_body").getElementsByClassName("item_title");
	var divs = $("right_body").querySelectorAll("h2.item_title");
	...
	mark_all_read: function(){
		...
		if(!list) return;
		if(list.length == 0){
			alert("There is no item to mark as read");
			return;
		}
		...
		var c = confirm(" are you sure to mark [[count]] feeds as read?".fill({
...
function prefetch(sid,count){
	...
	if(subs_item(sid).unread_count == 0 && Config.show_all == true){
		var api_url = feedly.BASE+"/streams/contents?streamId=" + encodeURIComponent(sid) + "&count=1&unreadOnly=false&ranked=newest";
		var api = new API(api_url);
		api.get({}, store_cache);
	} else {
...
function feed_discover(url){
	...
	var api = new API(feedly.BASE+"/search/feeds");
	api.get({url:url}, print_discover);
}
function feed_subscribe(feedlink,callback){
	var sid = "feed/"+feedlink;
...
Subscribe.Model = Class.create().extend({
	...
	make_subscribers_names: function(){
		...
		var subs_counts = this.list.pluck("subscribers");
	...
	update: function(reload_flag){
		...
		if((!reload_flag && this.loaded) || this.readyState >= 3){
			...
			var onload = function(tmp){
				if(canceled) return;
				self.readyState = 3;
				tmp = feedly.unreadCount2fastladder(tmp, Config.show_all);
			...
			var load_complete = function(){
				...
				setTimeout(function(){self.readyState=0},10000);
			};
			self.readyState = 1;
			load_request();
		}
...
Filter.created_on = function(v){return Filter.paddingLeft("posted: ")(Filter.toDate(v))};
Filter.modified_on = function(v){return Filter.paddingLeft(" | modified: ")(Filter.toDate(v))};
Filter.toDate = function(v){ return v ? new DateTime(v).toString() : "" };
Filter.toRelativeDate = function(v){return NowDate.rel( new DateTime(v) );};
...
function fit_screen(){
	...
	// if (browser.isMac && browser.isFirefox){ bottom_padding += 20; }
...
function print_discover(list){
	var list = feedly.discover2fastladder(list);
	var output = $("discover_items");
...
function get_folders(callback){
	...
	var api = new API(feedly.BASE+"/categories");
	api.get({},function(json){
		folder = feedly.categories2fastladder(json);
		// folder.id2name = {};
...
function get_first(id,callback){
	...
	if(get_unread.cache.has(id)){
		...
		api.get({}, function(data){
			var data = feedly.feed2fastladder(id,data)
...
/*
 未読の記事を読み込む
*/
LDR_VARS.PrefetchTimeout = 10000;
LDR_VARS.LockTimeout = 10000;
function get_unread(id,callback){
	...
	function no_cache(){
		...
		api.get({}, function(data){
			success = true;
			var tmp = feedly.feed2fastladder(id,data)
			get_unread.cache.set(id,tmp);
			print_feed.next(callback)(tmp);
		});
		// release lock
		setTimeout(function(){
...
function print_feed(feed){
	...
	var item_f = function(v){
		...
		return item_formatter(v,{
			relative_date : (v.created_on) ? ((Now-v.created_on)/1000).toRelativeDate() : "Unknown date",
...
function init_manage(){
	...
	ahah("manage.html","right_body",function(){
...
function init_config(){
	ahah("config.html", "right_body", function(){
...
function init_guide(){
	ahah("guide.html", "right_body", function(){
...
function init_ranking(){
	ahah("ranking.html", "right_body", function(){
		switchClass("right_container", "mode-ranking");
		Control.scroll_top();
		invoke_hook('AFTER_INIT_RANKING');
	});
}

function next_hint() {
	var hints = $("hint").getElementsByTagName('div');
	var len = hints.length;
	var n = 0;
	for (var i = 0; i < hints.length; i++) {
		if (hints[i].style.display == 'block') {
