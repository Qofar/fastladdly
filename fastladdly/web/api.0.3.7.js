...
API.extend({
	...
	post: function(param,onload){
		...
		var req = this.req;
		var postdata = Object.toJSON(param);
	...
	put: function(param,onload){
		...
		var req = this.req;
		var postdata = Object.toJSON(param);
		this.req.open("PUT",this.ap,true);
		this.req.setRequestHeader(
			"Content-Type", "application/json"
		);
		this.req.setRequestHeader(
		...
		this.req.onload = function(){
			...
			if(this.req.status !== 200){
				message("xhr err status: "+this.req.status);
				show_error();
			}else{
				onload({});
			}
	...
	delete: function(param,onload){
		...
		Object.extend(param);
		var postdata = Object.toQuery(param);
		// this.req.open("DELETE",this.ap + "?" + postdata,true);
		this.req.open("DELETE",this.ap,true);
		this.req.setRequestHeader(
			"Authorization", "OAuth " + feedly.access_token
		);
		this.req.onload = function(){
